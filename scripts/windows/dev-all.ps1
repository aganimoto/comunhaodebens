...
$DB = Join-Path $BACKEND "cdb_shalom.db"
...
if (-not (Test-Path $DB)) {
    Write-Host "[1/3] Criando banco SQLite via Alembic..." -ForegroundColor Yellow
...
Write-Host "[2/3] Iniciando WhatsApp Service (porta 3000)..." -ForegroundColor Yellow
...
Write-Host "[3/3] Iniciando Frontend (porta 5173)..." -ForegroundColor Yellow
